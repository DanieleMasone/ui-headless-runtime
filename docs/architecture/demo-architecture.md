...
Hash routing supports direct GitHub Pages refresh without server rewrites. The Command Palette controller powers client-side documentation search. Every component page uses the shared laboratory layout, `createExample()` production code, a safe state serializer, and a bounded event log.
