...
The local, stale-response, and empty-state examples execute directly from the component-specific [`combobox.ts` example module](https://github.com/DanieleMasone/ui-headless-runtime/blob/main/apps/demo/src/examples/combobox.ts); the async scenario deliberately gives the earlier query a longer latency.
