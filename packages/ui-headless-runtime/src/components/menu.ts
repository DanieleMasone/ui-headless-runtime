  ...
  return {
    ...
    handleKeyDown(event) {
      ...
      if (event.key === 'ArrowDown') move(1);
        ...
        if (submenuId) host.update({ ...host.getSnapshot(), openSubmenuId: submenuId });
...
export function createDropdownMenu(options: MenuOptions = {}): DropdownMenuController {
  ...
  return {
    ...menu,
    handleTrigger(event) {
      if (event instanceof KeyboardEvent && !['ArrowDown', 'Enter', ' '].includes(event.key))
        return;
      event.preventDefault();
      menu.open({ reason: event instanceof KeyboardEvent ? 'keyboard' : 'trigger', event });
    },
  };
}
...
export function createContextMenu(options: MenuOptions = {}): ContextMenuController {
  ...
  return {
    ...
    handleContextMenu(event, content) {
      ...
      return bindAt(anchor, event.currentTarget as HTMLElement, content);
    },
